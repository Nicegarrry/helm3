...
Use the explicit loopback port printed by the local demo/server. The CLI does not open the authority database or dispatch workers. The human-readable formatter and JSON output consume the same snapshot; remote origins, redirects and oversized responses are refused. The frozen demo receipt records three faux model requests, one workspace write, a succeeded command, lease-specific expiry refusal, scoped recovery evidence and the operator endpoint. The executable CLI also supports the optional `--serve` entry used by the demo; PR #59 remains under final review.
...
- [PR #59](https://github.com/Nicegarrry/helm3/pull/59): connected Fable/Astra fixture demo, final review pending at exact head `fa049ee6ad3497312bb49a268dbe7c4364a9b1c9`.
