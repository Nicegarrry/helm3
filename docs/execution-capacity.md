...
Released attempts stay fenced: queued commands cannot execute, new commands cannot join them, old effect identities are not replayed, and attempt-count and monetary limits remain in force. In-flight commands and unknown non-model effects still require reconciliation. A missing process handle, elapsed time, an agent's own claim, or an absent PID is not a substitute for the native stop receipt.
