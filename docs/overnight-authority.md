# Overnight build authority — 15 September 2026

The human approved continuing Wave 4 and onward fairly autonomously overnight and requested dogfooding Helm for native Pi workers, including available open models. The human supplied temporary OpenCode Go/Zen and Gemini credentials outside this document, and explicitly selected a US$10 combined spending maximum. Credential values must never enter Git, documentation, prompts, worker context or logs.

This authorises implementation of the existing Brief and addendum, scoped worker dispatch, independent executable review, reviewed PR integration, useful Map updates and bounded live provider tests through the supplied accounts. It supersedes the previous wave-only approval and provider-free-only restriction within these bounds. It does not change fixed product requirements or grant purchases, top-ups, unrelated deployment, reserve overrides or unlimited account use.

## Time and resource bounds

- Stop new autonomous work at 08:00 Australia/Sydney on 16 September 2026 (2026-09-15T22:00:00Z). Preserve active work, stop new spending and provide a morning handoff.
...
Continue the existing [Wave 4 sequence](wave4-plan.md), then the remaining Map outcomes as prerequisites become accepted. The orchestrator chooses the workflow; deterministic code owns effects, authority and evidence. Independent accepted PRs may merge without another general approval.
