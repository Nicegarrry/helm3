...
export class Helm {
  ...
  async spawn(input: SpawnInput): Promise<ToolOutcome<{ workerId: string; branch: string; worktree: string }>> {
    return this.spawnInternal(input);
  }
  ...
  async inspect(input: InspectInput): Promise<ToolOutcome<{
    ...
    try {
      ...
      let diffStat = '';
      try {
        diffStat = await this.workspace.diffStat(row.worktree, row.baseSha);
      } catch {
        diffStat = '';
      }
  ...
  async gate(input: GateInput): Promise<ToolOutcome<Omit<GateRow, 'gateId' | 'workerId' | 'at'>>> {
    try {
      const row = this.store.getWorker(input.workerId);
      if (!row) return refuse('worker not found');
      const clean = await this.workspace.isClean(row.worktree);
      if (!clean) return refuse('worktree is not clean');
  ...
  async prOpen(input: PrOpenInput): Promise<ToolOutcome<{ number: number; url: string; head: string }>> {
    try {
      const row = this.store.getWorker(input.workerId);
      if (!row) return refuse('worker not found');
      if (!row.head) return refuse('worker has no commits yet');
      const passing = this.store.listGates(input.workerId).filter((g) => g.head === row.head && g.passed);
      if (passing.length === 0) return refuse(`no passing gate at head ${row.head}`);
    ...
    try {
      ...
      if (number !== undefined) {
        ...
        if (worker) {
          ...
          const pr = this.store.getPrByNumber(number);
          if (!pr) return refuse('pr not found');
          const w = this.store.getWorker(pr.workerId);
          if (!w) return refuse('pr worker not found');
          repoSlug = w.repoSlug;
        }
      } else {
        if (!input.workerId) return refuse('number or workerId required');
        const pr = this.store.getPrByWorker(input.workerId);
        if (!pr) return refuse('no pr for worker');
        number = pr.number;
        const w = this.store.getWorker(input.workerId);
        if (!w) return refuse('worker not found');
        repoSlug = w.repoSlug;
      }
  ...
  async reviewRequest(input: ReviewInput): Promise<ToolOutcome<{ reviewWorkerId: string }>> {
    try {
      let pr: PrRow | undefined;
      if (input.number !== undefined) pr = this.store.getPrByNumber(input.number);
      else if (input.workerId) pr = this.store.getPrByWorker(input.workerId);
      if (!pr) return refuse('pr not found');
      const confirmedPr = pr;
      const sourceWorker = this.store.getWorker(confirmedPr.workerId);
      if (!sourceWorker) return refuse('source worker not found');
      const objective = `Review PR #${confirmedPr.number} (${confirmedPr.url}) on branch ${sourceWorker.branch} in ${sourceWorker.repoSlug}. Read the diff, run relevant checks, and report findings as the worker result.`;
      const spawnPayload: SpawnInput = {
        repo: sourceWorker.repo,
        objective,
        model: input.model,
        baseRef: sourceWorker.branch,
        role: 'reviewer',
        contextPaths: [],
      ...
      const onDone: OnDone = async (workerId, result) => {
        ...
        await this.github.comment(sourceWorker.repoSlug, confirmedPr.number, body);
        this.store.appendEvent(workerId, 'review.posted', { number: confirmedPr.number });
      };
      const outcome = await this.spawnInternal(spawnPayload, onDone);
  ...
  async prMerge(input: PrMergeInput): Promise<ToolOutcome<{ merged: true }>> {
    try {
      const pr = this.store.getPrByNumber(input.number);
      if (!pr) return refuse('pr not found');
      const worker = this.store.getWorker(pr.workerId);
      if (!worker) return refuse('pr worker not found');
      const status = await this.github.prStatus(worker.repoSlug, input.number);
      if (status.state !== 'open') return refuse(`pr is ${status.state}, not open`);
      if (status.mergeable !== true) return refuse('pr is not mergeable');
      if (status.head !== input.expectedHead) return refuse(`head mismatch: expected ${input.expectedHead}, got ${status.head}`);
  ...
  private startRun(workerId: string, message: string, onDone?: OnDone): void {
    ...
    void promise.finally(() => {
      if (this.running.get(workerId) === promise) this.running.delete(workerId);
    });
  }
  ...
  private async runTurn(workerId: string, message: string, onDone?: OnDone): Promise<void> {
    ...
    try {
      ...
      this.store.updateWorker(workerId, {
        ...
        result,
        rawResultText: result === null ? outcome.rawText : null,
      });
