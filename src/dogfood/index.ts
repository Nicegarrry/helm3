...
export async function runLocalFixture(options: LocalFixtureOptions): Promise<LocalFixtureResult> {
  ...
  const forkPayload = z.object({ workerId: z.string().min(1), attemptId: z.string().min(1), predecessorWorkerId: z.string().min(1), expectedSessionId: z.string().min(1), expectedHead: z.string().regex(/^[0-9a-f]{40}$/), sourceSessionId: z.string().min(1), sourceHistoryHash: z.string().regex(/^sha256:[0-9a-f]{64}$/), sourceBranchDigest: z.string().regex(/^sha256:[0-9a-f]{64}$/), inputDigest: z.string().min(1), modelId: z.literal('offline'), modelProvider: z.literal('helm3-local-faux'), modelApi: z.string().min(1), modelFactVersion: z.literal(1), dataPolicy: z.literal('public-only') }).strict();
