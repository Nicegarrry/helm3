...
import { PiWorkerFleet, type WorkerForkInput, type WorkerSpawnInput, type WorkerSteerInput } from '../host/worker-fleet.js';
...
/** `native-fork` is a bounded dogfood scenario, not an orchestration workflow option. */
export type LocalFixtureOptions = Readonly<{ stateDirectory: string; orchestrator: 'fable' | 'astra'; scenario?: 'default' | 'native-fork' }>;
...
export async function runLocalFixture(options: LocalFixtureOptions): Promise<LocalFixtureResult> {
  ...
  const nativeForkScenario = options.scenario === 'native-fork';
  ...
  const steerPayload = z.object({ workerId: z.string().min(1), attemptId: z.string().min(1), predecessorWorkerId: z.string().min(1), objectiveRef: z.string().min(1), evidenceRefs: z.array(z.string().min(1)).min(1), gateCommandId: z.string().min(1).optional(), expectedSessionId: z.string().min(1), expectedHead: z.string().regex(/^[0-9a-f]{40}$/), inputDigest: z.string().min(1), modelId: z.literal('offline'), modelProvider: z.literal('helm3-local-faux'), modelApi: z.string().min(1), modelFactVersion: z.literal(1), dataPolicy: z.literal('public-only') }).strict();
  const forkPayload = z.object({ workerId: z.string().min(1), attemptId: z.string().min(1), predecessorWorkerId: z.string().min(1), expectedSessionId: z.string().min(1), expectedHead: z.string().regex(/^[0-9a-f]{40}$/), sourceSessionId: z.string().min(1), sourceHistoryHash: z.string().regex(/^sha256:[0-9a-f]{64}$/), sourceBranchDigest: z.string().regex(/^sha256:[0-9a-f]{64}$/), sourceLeafId: z.string().min(1), destination: z.string().min(1), ownerAttemptId: z.string().min(1), ownerGeneration: z.number().int().positive(), ownerExpiresAt: z.string().min(1), inputDigest: z.string().min(1), modelId: z.literal('offline'), modelProvider: z.literal('helm3-local-faux'), modelApi: z.string().min(1), modelFactVersion: z.literal(1), dataPolicy: z.literal('public-only') }).strict();
  const kinds = { 'worker.spawn': { payloadSchema: workerPayload, modelSelection: (value: unknown) => { const selected = workerPayload.parse(value); return { modelId: selected.modelId, provider: selected.modelProvider, factVersion: selected.modelFactVersion, role: selected.role, requiredCapabilities: ['build'], dataClassification: 'public' as const }; } }, 'worker.steer': { payloadSchema: steerPayload, modelSelection: (value: unknown) => { const selected = steerPayload.parse(value); return { modelId: selected.modelId, provider: selected.modelProvider, factVersion: selected.modelFactVersion, role: 'builder', requiredCapabilities: ['build'], dataClassification: 'public' as const }; } }, 'worker.fork': { payloadSchema: forkPayload, modelSelection: (value: unknown) => { const selected = forkPayload.parse(value); return { modelId: selected.modelId, provider: selected.modelProvider, factVersion: selected.modelFactVersion, role: 'builder', requiredCapabilities: ['build'], dataClassification: 'public' as const }; } }, 'worker.stop': { payloadSchema: z.object({ workerId: z.string().min(1) }).strict() }, 'gate.run': { payloadSchema: gateRunPayloadSchema }, 'map.update': { payloadSchema: mapUpdatePayloadSchema }, 'map.close': { payloadSchema: mapClosePayloadSchema }, 'pi.model': { payloadSchema: piPayload, resourceRequest: () => ({ poolId: 'fixture-requests', unit: 'requests', upperBound: 1, consumer: 'worker' as const }) }, 'pi.write': { payloadSchema: piPayload } };
  plane = await openHost({ stateDirectory: join(root, 'host'), now: () => clock, kinds });
  plane.recordHumanAuthority({ authorityId: 'fixture-human', repositoryId: 'fixture/repository', mapNodeIds: ['2'], allowedActions: ['worker.spawn', 'worker.steer', 'worker.fork', 'worker.stop', 'gate.run', 'map.update', 'map.close', 'pi.model', 'pi.write'], expiresAt: ownerLater, maxConcurrency: 2, maxAttemptsPerNode: 4, poolLimits: [{ poolId: 'fixture-requests', unit: 'requests', limit: 4 }], protectedReserves: [] });
  plane.recordAutonomyLease({ leaseId: 'fixture-autonomy', revision: 1, issuedBy: 'fixture-human', parentAuthorityId: 'fixture-human', scope: { repositoryId: 'fixture/repository', mapNodeIds: ['2'] }, allowedActions: ['worker.spawn', 'worker.steer', 'worker.fork', 'worker.stop', 'gate.run', 'map.update', 'map.close', 'pi.model', 'pi.write'], issuedAt: now, expiresAt: later, maxConcurrency: 2, maxAttemptsPerNode: 4, poolLimits: [{ poolId: 'fixture-requests', unit: 'requests', limit: 4 }], protectedReserves: [] });
  ...
  const fleet = new PiWorkerFleet({ host: plane, workspaceManager, executor: { executorId: 'fixture-host' }, claimExpiresAt: () => later, readFact: async () => ({ value: true, state: 'known', source: 'fixture', observedAt: now }),
    ...
    steerCommand: (input: WorkerSteerInput, successorWorkerId: string, record, nextAttemptId: string, context: HelmToolExecutionContext) => { activeContext = context; activeAttemptId = nextAttemptId; const frozenInput = { ...input, evidenceRefs: [...input.evidenceRefs] }; const payload = { workerId: successorWorkerId, attemptId: nextAttemptId, predecessorWorkerId: record.workerId, objectiveRef: input.objectiveRef, evidenceRefs: [...input.evidenceRefs], ...(input.gateCommandId ? { gateCommandId: input.gateCommandId } : {}), expectedSessionId: input.expectedSessionId, expectedHead: input.expectedHead, inputDigest: hash(frozenInput), modelId: 'offline' as const, modelProvider: record.modelProvider, modelApi: record.modelApi, modelFactVersion: 1 as const, dataPolicy: 'public-only' as const }; return { schemaVersion: 1, commandId: `fixture-worker-steer-${nextAttemptId}`, kind: 'worker.steer', idempotencyKey: `fixture-worker-steer-${nextAttemptId}`, payloadHash: hash(payload), scope: { repositoryId: 'fixture/repository', mapNodeId: '2' }, actorId: 'fixture-model', runId: context.runId, origin: 'orchestrator', leaseId: 'fixture-autonomy', leaseRevision: 1, orchestratorLeaseId: 'fixture-orchestrator', orchestratorEpoch: 1, plannedAt: now, notAfter: later, expected: [], payload, requiredEvidence: [] } as Command; },
    forkCommand: (input: WorkerForkInput, successorWorkerId: string, record, nextAttemptId: string, source, context: HelmToolExecutionContext) => { activeContext = context; activeAttemptId = nextAttemptId; forkedWorkerIds.add(successorWorkerId); const payload = { workerId: successorWorkerId, attemptId: nextAttemptId, predecessorWorkerId: record.workerId, expectedSessionId: input.expectedSessionId, expectedHead: input.expectedHead, sourceSessionId: source.sessionId, sourceHistoryHash: source.historyHash, sourceBranchDigest: source.branchDigest, sourceLeafId: source.leafId, destination: join(root, 'fork-worker'), ownerAttemptId: nextAttemptId, ownerGeneration: 1, ownerExpiresAt: later, inputDigest: hash(input), modelId: 'offline' as const, modelProvider: record.modelProvider, modelApi: record.modelApi, modelFactVersion: 1 as const, dataPolicy: 'public-only' as const }; return { schemaVersion: 1, commandId: `fixture-worker-fork-${nextAttemptId}`, kind: 'worker.fork', idempotencyKey: `fixture-worker-fork-${nextAttemptId}`, payloadHash: hash(payload), scope: { repositoryId: 'fixture/repository', mapNodeId: '2' }, actorId: 'fixture-model', runId: context.runId, origin: 'orchestrator', leaseId: 'fixture-autonomy', leaseRevision: 1, orchestratorLeaseId: 'fixture-orchestrator', orchestratorEpoch: 1, plannedAt: now, notAfter: later, expected: [], payload, requiredEvidence: [] } as Command; },
    ...
    attempt: (command, workerId) => ({ attemptId: activeAttemptId, mapNodeId: '2', mapNodeRevision: 'fixture', objectiveVersion: 'fixture', acceptanceVersion: 'fixture', role: 'builder', model: 'offline', family: 'faux', provider: 'helm3-local-faux', capability: 'fixture', poolId: 'fixture-requests', workspace: command.kind === 'worker.fork' || (command.kind === 'worker.steer' && forkedWorkerIds.has((command.payload as { predecessorWorkerId?: string }).predecessorWorkerId ?? '')) ? join(root, 'fork-worker') : join(root, 'worker'), baseSha: (command.payload as { expectedHead?: string }).expectedHead ?? baseSha, contextManifestHash: 'sha256:fixture-context', leaseId: 'fixture-autonomy', sessionIds: [], commandIds: [command.commandId], startedAt: now, evidenceRefs: [], usageRefs: [], findingRefs: [] }),
    workspace: (command, _workerId, attempt) => ({ repository, destination: command.kind === 'worker.steer' && workspace ? workspace.root : attempt.workspace, branch: command.kind === 'worker.fork' ? 'fixture-fork' : 'fixture-worker', baseSha: (command.payload as { expectedHead?: string }).expectedHead ?? baseSha, owner: { attemptId: attempt.attemptId, generation: command.kind === 'worker.steer' ? 2 : 1, expiresAt: later }, policy: { writableRoots: ['.'], protectedRoots: ['README.md'] } }),
    ...
    rehydrate: async (command, reservation, persisted) => { workspace = reservation; const transcript = await readFile(persisted.sessionFile, 'utf8'); secondModelSawPriorContext = transcript.includes('bad fixture result') && transcript.includes('fixture wrote invalid result.txt'); const authority = plane!.piAuthority({ attemptId: activeAttemptId, actorId: 'fixture-pi', executorId: 'fixture-pi', commandForEffect: (effect) => piCommand(effect), observedSettlement: (effect) => effect.kind === 'model.request' ? { state: 'known', amount: 1 } : undefined }); return PiNativeWorker.rehydrate({ commandId: command.commandId, attemptId: activeAttemptId, workspace: reservation, owner: reservation.owner, workspaceManager, authority, journal: plane!.artifactsFor(activeContext!).journalForTrustedPi(), stateRoot: join(root, 'pi-state'), modelRuntime: runtime, model: faux.getModel() }, persisted); },
    forkSourceSnapshot: (persisted, reservation) => PiNativeWorker.inspectForkSource({ workspace: reservation, workspaceManager, owner: reservation.owner, stateRoot: join(root, 'pi-state') }, persisted),
    fork: async (command, reservation, persisted, expectedLeafId) => { workspace = reservation; const authority = plane!.piAuthority({ attemptId: activeAttemptId, actorId: 'fixture-pi', executorId: 'fixture-pi', commandForEffect: (effect) => piCommand(effect), observedSettlement: (effect) => effect.kind === 'model.request' ? { state: 'known', amount: 1 } : undefined }); const forked = await PiNativeWorker.forkAtCurrentTip({ commandId: command.commandId, attemptId: activeAttemptId, workspace: reservation, owner: reservation.owner, workspaceManager, authority, journal: plane!.artifactsFor(activeContext!).journalForTrustedPi(), stateRoot: join(root, 'pi-state'), modelRuntime: runtime, model: faux.getModel() }, persisted, expectedLeafId); return { worker: forked.worker, successor: forked.successor, leafId: forked.leafId }; },
  ...
  let fixtureForkInput: WorkerForkInput | undefined;
  ...
  const fixtureTurns = [
    ...
    ...(nativeForkScenario ? [() => ({ name: 'worker.fork', input: fixtureForkInput })] : []),
  ...
  if (options.orchestrator === 'fable') {
    ...
    const sdk = { ...claude, createSdkMcpServer(input: Parameters<typeof claude.createSdkMcpServer>[0]) { definitions = input.tools as typeof definitions; return claude.createSdkMcpServer(input); }, query: (() => (async function* () { const turn = fixtureTurns[invocation++]; const action = turn?.(); const definition = action && definitions.find((entry) => entry.name === action.name); const result = definition && action?.input ? await definition.handler(action.input as Record<string, unknown>, {}) : undefined; if ((result as { structuredContent?: { state?: string; reason?: string } } | undefined)?.structuredContent?.state === 'refused') throw new Error((result as { structuredContent: { reason?: string } }).structuredContent.reason); yield { type: 'system', subtype: 'init', session_id: 'fixture-fable' }; yield { type: 'result', subtype: 'success', session_id: 'fixture-fable', result }; })()) as unknown as typeof claude.query };
...
const fixtureTurn = Number.parseInt((() => { try { return readFileSync(${JSON.stringify(join(root, 'fixture-driver-count'))}, 'utf8'); } catch { return '0'; } })(), 10); if (fixtureTurn < ${nativeForkScenario ? 7 : 6}) process.argv = process.argv.map((value) => value.replaceAll('resume', 'continued'));
const args = process.argv.join(' '); const resumed = args.includes('resume'); const url = args.match(/http:\\/\\/127\\.0\\.0\\.1:[0-9]+\\/mcp/)?.[0]; const key = Object.keys(process.env).find((name) => name.startsWith('HELM_ASTRA_MCP_TOKEN_')); if (!url || !key) throw new Error('fixture MCP config was absent'); const transport = new StreamableHTTPClientTransport(new URL(url), { requestInit: { headers: { authorization: 'Bearer ' + process.env[key] } } }); const client = new Client({ name: 'fixture-codex', version: '1' }); await client.connect(transport); const counterPath = ${JSON.stringify(join(root, 'fixture-driver-count'))}; const count = Number.parseInt((() => { try { return readFileSync(counterPath, 'utf8'); } catch { return '0'; } })(), 10); const turns = JSON.parse(readFileSync(${JSON.stringify(join(root, 'fixture-driver-turns.json'))}, 'utf8')); const turn = turns[count]; const response = resumed || !turn ? undefined : await client.callTool({ name: turn.name, arguments: JSON.parse(readFileSync(turn.inputPath, 'utf8')) }); if (!resumed && turn) writeFileSync(counterPath, String(count + 1)); await transport.close(); if (response?.isError) throw new Error('fixture MCP tool was refused'); console.log(JSON.stringify({ type: 'thread.started', thread_id: 'fixture-astra' })); console.log(JSON.stringify({ type: 'turn.completed', usage: { input_tokens: 0, cached_input_tokens: 0, cache_write_input_tokens: 0, output_tokens: 0, reasoning_output_tokens: 0 } }));\n`, { mode: 0o700 }); await chmod(executable, 0o700);
  ...
  await writeFile(join(root, 'fixture-driver-turns.json'), JSON.stringify([
    ...
    ...(nativeForkScenario ? [{ name: 'worker.fork', inputPath: join(root, 'fixture-fork-input.json') }] : []),
  ...
  if (nativeForkScenario) {
    fixtureForkInput = Object.freeze({ workerId: spawnedId, expectedSessionId: spawnedWorker.sessionId, expectedHead: gateHead });
    await writeFile(join(root, 'fixture-fork-input.json'), JSON.stringify(fixtureForkInput), { mode: 0o600 });
    await driver.invoke({ sessionId: started.sessionId, objectiveRef, contextRefs: [] });
    const forked = (await plane.snapshot(runId)).commands.filter((entry) => entry.command.kind === 'worker.fork').at(-1);
    const forkWorkerId = (forked?.command.payload as { workerId?: string } | undefined)?.workerId;
    if (!forked || !forkWorkerId || forked.status !== 'succeeded') throw new Error('fixture native worker fork was not durably observed');
    const forkedWorker = await fleet.inspect({ runId, sessionId: started.sessionId, mode: 'primary' }, forkWorkerId);
    if (forkedWorker.state !== 'fork_ready' || forkedWorker.sessionId === spawnedWorker.sessionId) throw new Error('fixture native fork was not an idle distinct session');
    fixtureSteerInput = Object.freeze({ workerId: forkWorkerId, objectiveRef, evidenceRefs: [objectiveRef], expectedSessionId: forkedWorker.sessionId, expectedHead: gateHead });
  } else fixtureSteerInput = Object.freeze({ workerId: spawnedId, objectiveRef, evidenceRefs: redGateEvidenceRefs, gateCommandId: redGate.command.commandId, expectedSessionId: spawnedWorker.sessionId, expectedHead: gateHead });
  ...
  if (!steered || !steerWorkerId || !steerAttemptId || steered.status !== 'succeeded') throw new Error(`fixture worker steer did not persist its successor identity: ${JSON.stringify(steered)}`);
