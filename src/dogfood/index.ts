...
export async function runLocalFixture(options: LocalFixtureOptions): Promise<LocalFixtureResult> {
  ...
  const forkPayload = z.object({ workerId: z.string().min(1), attemptId: z.string().min(1), predecessorWorkerId: z.string().min(1), expectedSessionId: z.string().min(1), expectedHead: z.string().regex(/^[0-9a-f]{40}$/), sourceSessionId: z.string().min(1), sourceHistoryHash: z.string().regex(/^sha256:[0-9a-f]{64}$/), sourceBranchDigest: z.string().regex(/^sha256:[0-9a-f]{64}$/), destination: z.string().min(1), ownerAttemptId: z.string().min(1), ownerGeneration: z.number().int().positive(), ownerExpiresAt: z.string().min(1), inputDigest: z.string().min(1), modelId: z.literal('offline'), modelProvider: z.literal('helm3-local-faux'), modelApi: z.string().min(1), modelFactVersion: z.literal(1), dataPolicy: z.literal('public-only') }).strict();
  ...
  const fleet = new PiWorkerFleet({ host: plane, workspaceManager, executor: { executorId: 'fixture-host' }, claimExpiresAt: () => later, readFact: async () => ({ value: true, state: 'known', source: 'fixture', observedAt: now }),
    ...
    fork: async (command, reservation, persisted) => { workspace = reservation; const authority = plane!.piAuthority({ attemptId: activeAttemptId, actorId: 'fixture-pi', executorId: 'fixture-pi', commandForEffect: (effect) => piCommand(effect), observedSettlement: (effect) => effect.kind === 'model.request' ? { state: 'known', amount: 1 } : undefined }); const forked = await PiNativeWorker.forkAtCurrentTip({ commandId: command.commandId, attemptId: activeAttemptId, workspace: reservation, owner: reservation.owner, workspaceManager, authority, journal: plane!.artifactsFor(activeContext!).journalForTrustedPi(), stateRoot: join(root, 'pi-state'), modelRuntime: runtime, model: faux.getModel() }, persisted); return { worker: forked.worker, successor: forked.successor, leafId: forked.leafId }; },
