...
test('native fleet terminal events signal once, preserve a delivery fault for reopen replay, and never run a new worker', async () => {
  ...
  let starts = 0; let runs = 0; let outcomes: Array<'success' | 'unknown'> = [];
  try {
    ...
    const binding = () => ({ host: plane!, workspaceManager: workspace!, executor: { executorId: 'fleet' }, claimExpiresAt: () => later, readFact: async () => ({ value: true, state: 'known' as const, source: 'fixture', observedAt: stamp }),
      ...
      start: async () => { starts++; const outcome = outcomes.shift(); if (!outcome) throw new Error('fixture outcome is absent'); const worker = { sessionId: `pi-${starts}`, isActive: true, modelIdentity: { modelId: 'offline', provider: 'faux', api: 'fixture' }, contextOccupancy: {}, async run() { runs++; (this as { isActive: boolean }).isActive = false; if (outcome === 'unknown') throw new Error('native runner failed without a terminal envelope'); return { result: { status: 'succeeded' }, artifacts: [] }; }, async persistedSession() { return { sessionId: `pi-${starts}`, sessionFile: 'fixture', historyHash: `sha256:${'a'.repeat(64)}`, branchDigest: `sha256:${'b'.repeat(64)}` }; }, async stopLocal() { return 'stopped' as const; }, dispose() {} }; return worker as unknown as PiNativeWorker; },
    ...
    assert.equal(fleetEvents.filter(event => event.kind === 'worker.completed' || event.kind === 'worker.failed').length, 2); assert.equal(signals.length, 2); assert.equal(wakes.length, 1); assert.equal((unknownSignal.payload as { evidenceRefs: string[] }).evidenceRefs.length, 1); assert.equal(starts, 2); assert.equal(runs, 2, 'replay does not start or run Pi');
