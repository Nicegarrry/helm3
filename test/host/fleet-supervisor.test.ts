...
test('native fleet terminal events signal once, preserve a delivery fault for reopen replay, and never run a new worker', async () => {
  ...
  let starts = 0; let runs = 0; let outcomes: Array<'success' | 'failed' | 'unknown'> = [];
  try {
    ...
    await open();
    plane.recordHumanAuthority({ authorityId: 'human', repositoryId: 'repo', mapNodeIds: ['node'], allowedActions: ['worker.spawn'], expiresAt: later, maxConcurrency: 4, maxAttemptsPerNode: 4, poolLimits: [], protectedReserves: [] });
    plane.recordAutonomyLease({ leaseId: 'auto', revision: 1, issuedBy: 'human', parentAuthorityId: 'human', scope: { repositoryId: 'repo', mapNodeIds: ['node'] }, allowedActions: ['worker.spawn'], issuedAt: stamp, expiresAt: later, maxConcurrency: 4, maxAttemptsPerNode: 4, poolLimits: [], protectedReserves: [] });
    ...
    const binding = () => ({ host: plane!, workspaceManager: workspace!, executor: { executorId: 'fleet' }, claimExpiresAt: () => later, readFact: async () => ({ value: true, state: 'known' as const, source: 'fixture', observedAt: stamp }),
      ...
      start: async (command: Command) => { starts++; const outcome = outcomes.shift(); if (!outcome) throw new Error('fixture outcome is absent'); const attemptId = (command.payload as { attemptId: string }).attemptId; const result = { status: outcome === 'failed' ? 'failed' as const : 'succeeded' as const, summary: 'fixture', changed_files: [], commits: [], decisions: [], discoveries: [], tests_claimed: [], acceptance_claims: [], risks: [], unresolved: [], artifacts: [], recommended_next_action: 'fixture complete' }; const envelope = outcome === 'unknown' ? undefined : await plane!.artifactsFor(context).journalForTrustedPi().append({ source: 'pi.envelope', sourceIdentity: `pi-envelope:${attemptId}:fixture:terminal`, mediaType: 'application/json', bytes: Buffer.from(JSON.stringify(result)) }); const worker = { sessionId: `pi-${starts}`, isActive: true, modelIdentity: { modelId: 'offline', provider: 'faux', api: 'fixture' }, contextOccupancy: {}, async run() { runs++; (this as { isActive: boolean }).isActive = false; if (outcome === 'unknown') throw new Error('native runner failed without a terminal envelope'); return { result, artifacts: [envelope!] }; }, async persistedSession() { return { sessionId: `pi-${starts}`, sessionFile: 'fixture', historyHash: `sha256:${'a'.repeat(64)}`, branchDigest: `sha256:${'b'.repeat(64)}` }; }, async stopLocal() { return 'stopped' as const; }, dispose() {} }; return worker as unknown as PiNativeWorker; },
    ...
    assert.equal(fleetEvents.filter(event => event.kind === 'worker.completed' || event.kind === 'worker.failed').length, 3); assert.equal(signals.length, 3); assert.equal(wakes.length, 2); assert.equal((unknownSignal.payload as { evidenceRefs: string[] }).evidenceRefs.length, 2, 'the signal retains both the prior launch chain and immutable terminal phase evidence'); assert.equal(starts, 3); assert.equal(runs, 3, 'replay does not start or run Pi');
