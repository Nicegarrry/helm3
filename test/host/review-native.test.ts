...
import { IndependentReviewService, type ReviewSource } from '../../src/host/review.js';
...
test('native faux independent review uses only helm_read in a fresh readonly Pi worktree', async () => {
  ...
  try {
    ...
    assert.notEqual(launch.attemptId, source.attemptId); assert.notEqual(launch.sessionId, source.sessionId);
    await fleet.waitForTerminal(launch.workerId);
    const inspected = await fleet.inspect(context, launch.workerId); assert.equal(inspected.state, 'terminal', JSON.stringify((await plane.snapshot('run')).commands.map(item => ({ kind: item.command.kind, status: item.status, observations: item.observations })))); const beforeAfter = await Promise.all([exec('git', ['-C', inspected.workspace, 'rev-parse', 'HEAD']), exec('git', ['-C', inspected.workspace, 'status', '--porcelain', '--untracked-files=all'])]);
