...
test('review observer accepts only terminal native evidence bound to its reviewer and exact unchanged Git snapshot', async () => {
  ...
  try {
    ...
    const incompleteResult = await journal.append({ source: 'pi.envelope', sourceIdentity: 'pi-envelope:attempt-reviewer:invocation:initial', mediaType: 'text/plain', bytes: Buffer.from(JSON.stringify({ status: 'succeeded' })) });
    const result = await journal.append({ source: 'pi.envelope', sourceIdentity: 'pi-envelope:attempt-reviewer:invocation:corrected', mediaType: 'text/plain', bytes: Buffer.from(JSON.stringify({ status: 'succeeded', summary: 'reviewed', changed_files: [], commits: [], decisions: [], discoveries: [], tests_claimed: [], acceptance_claims: [], risks: [], unresolved: [], artifacts: [], recommended_next_action: 'report' })) });
    ...
    const fleet = { terminalJournal: async () => ({ workerId: 'worker-reviewer', attemptId: 'attempt-reviewer', sessionId: 'session-reviewer', spawnCommandId: 'spawn-reviewer', workspace: reservation.root, owner: reservation.owner, modelId: 'reviewer', modelProvider: 'faux', modelApi: 'fixture', evidenceRefs: [incompleteResult.ref, result.ref, events.ref], reviewBeforeRef: beforeRaw.ref }) } as unknown as PiWorkerFleet;
