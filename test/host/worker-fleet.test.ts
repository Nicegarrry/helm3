...
test('worker tools create a short setup effect, remain inspectable while running, and report restart as live unknown', async () => {
  ...
  try {
    ...
    const payload = z.object({ workerId: z.string(), attemptId: z.string(), modelId: z.literal('offline'), role: z.literal('builder') }).strict();
    ...
    const start = await registry.invoke('worker.spawn', { objectiveRef: 'o', acceptanceRef: 'a', contextRefs: [], modelId: 'offline', role: 'builder' }, context);
