...
async function setup(options: { successorWait?: Deferred; successorModel?: string; successorPersistenceFailure?: boolean } = {}) {
  ...
  const spawnPayload = z.object({ workerId: z.string(), attemptId: z.string(), modelId: z.literal('offline'), role: z.literal('builder'), inputDigest: z.string(), baseSha: z.string(), modelFactVersion: z.literal(1), dataPolicy: z.literal('public-only') }).strict();
  const steerPayload = z.object({ workerId: z.string(), attemptId: z.string(), predecessorWorkerId: z.string(), inputDigest: z.string(), modelId: z.literal('offline'), modelFactVersion: z.literal(1), dataPolicy: z.literal('public-only'), expectedHead: z.string() }).strict();
  ...
  const binding = () => ({ host: plane, workspaceManager: workspace, executor: { executorId: 'fleet' }, claimExpiresAt: () => '2200-01-01T00:00:00Z', readFact: async () => {
    ...
    spawnCommand(input: WorkerSpawnInput, workerId: string, attemptId: string): Command { const payload = { workerId, attemptId, modelId: input.modelId, role: input.role, inputDigest: hash({ ...input, contextRefs: [...input.contextRefs] }), baseSha, modelFactVersion: 1 as const, dataPolicy: 'public-only' as const }; return command(`spawn-${workerId}`, 'worker.spawn', payload, attemptId); },
    steerCommand(input: WorkerSteerInput, workerId: string, previous: { workerId: string; modelId: string; modelFactVersion: number; dataPolicy: string }, attemptId: string): Command { const payload = { workerId, attemptId, predecessorWorkerId: previous.workerId, inputDigest: hash({ ...input, evidenceRefs: [...input.evidenceRefs] }), modelId: previous.modelId, modelFactVersion: previous.modelFactVersion, dataPolicy: previous.dataPolicy, expectedHead: input.expectedHead }; return command(`steer-${workerId}`, 'worker.steer', payload, attemptId); },
    inputDigest: (command: Command) => (command.payload as { inputDigest: string }).inputDigest, stopCommand: () => { throw new Error('unused'); },
    attempt(command: Command, workerId: string): Attempt { const payload = command.payload as { attemptId: string; baseSha?: string; expectedHead?: string }; return { attemptId: payload.attemptId, mapNodeId: 'node', mapNodeRevision: '1', objectiveVersion: '1', acceptanceVersion: '1', role: 'builder', model: 'offline', family: 'faux', provider: 'faux', capability: 'build', poolId: 'none', workspace: join(root, 'worker'), baseSha: payload.baseSha ?? payload.expectedHead!, contextManifestHash: 'sha256:context', leaseId: 'auto', sessionIds: [], commandIds: [command.commandId], startedAt: stamp, evidenceRefs: [], usageRefs: [], findingRefs: [] }; },
    workspace(command: Command, _workerId: string, attempt: Attempt) { const steer = command.kind === 'worker.steer'; return { repository: repo, destination: join(root, 'worker'), branch: 'worker', baseSha: attempt.baseSha, owner: { attemptId: attempt.attemptId, generation: steer ? 2 : 1, expiresAt: later }, policy: { writableRoots: ['.'] } }; },
    start: async () => worker('pi-session'), rehydrate: async () => { rehydrates++; return worker('pi-session', options.successorWait, options.successorModel ?? 'offline', options.successorPersistenceFailure); }, prompt: () => 'fixture', correction: () => 'fixture' });
...
test('a mismatched native continuation model records unknown rather than starting a successor runner', async () => { const f = await setup({ successorModel: 'wrong' }); try { await assert.rejects(f.fleet.steer(f.context, steerInput(f)), /durably observed/); const record = (await f.plane.snapshot('run')).commands.find((item) => item.command.kind === 'worker.steer'); assert.equal(record?.status, 'unknown'); assert.match(record?.observations.at(-1)?.detail ?? '', /identity changed/); assert.equal(f.rehydrates(), 1); } finally { await f.cleanup(); } });

test('at-effect Git, lease, and epoch rechecks prevent a successor from rehydrating', async () => { for (const fault of ['head', 'lease', 'epoch'] as const) { const f = await setup(); try { f.armEffectFault(fault); await assert.rejects(f.fleet.steer(f.context, steerInput(f)), /durably observed|lease|owner|epoch/); const record = (await f.plane.snapshot('run')).commands.find((item) => item.command.kind === 'worker.steer'); assert.equal(f.rehydrates(), 0, `${fault} must not reach Pi rehydrate`); assert.equal(record?.status, fault === 'head' ? 'unknown' : 'refused'); } finally { await f.cleanup(); } } });
