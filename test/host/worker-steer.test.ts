...
async function setup(options: { successorWait?: Deferred; successorModel?: string; successorPersistenceFailure?: boolean; registryProvider?: string; forkReceiptFailure?: boolean; forkEventFailure?: boolean } = {}) {
  ...
  const forkPayload = steerPayload.extend({ sourceSessionId: z.string(), sourceHistoryHash: z.string().regex(/^sha256:[0-9a-f]{64}$/), sourceBranchDigest: z.string().regex(/^sha256:[0-9a-f]{64}$/) }).strict();
