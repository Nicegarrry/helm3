...
test('GitHub gateway pins the REST merge compare-and-swap SHA and reads CI by exact SHA', async () => {
  const calls: string[][] = [];
  const transport = async (args: readonly string[]) => { calls.push([...args]); const path = args.find(a => a.startsWith('repos/')) ?? ''; if (path.endsWith('/pulls/8')) return { ok: true, stdout: JSON.stringify({ state: 'open', mergeable_state: 'clean', head: { sha: head }, base: { ref: 'main', sha: 'c'.repeat(40) } }), stderr: '' }; if (path.endsWith('/check-runs')) return { ok: true, stdout: JSON.stringify({ check_runs: [{ status: 'completed', conclusion: 'success' }] }), stderr: '' }; if (path.endsWith('/status')) return { ok: true, stdout: JSON.stringify({ statuses: [{ state: 'success' }] }), stderr: '' }; if (path.includes('/git/ref/')) return { ok: true, stdout: JSON.stringify({ object: { sha: 'c'.repeat(40) } }), stderr: '' }; return { ok: true, stdout: JSON.stringify({ merged: true }), stderr: '' }; };
  const live = createGitHubIntegrationGateway({ repository: 'acme/helm', receipts: () => [receipt], acceptanceEvidence: () => [{ ref: 'gate:8', head }], transport }); const observed = await live.read(8); await live.merge(8, head);
  assert.equal(observed.state, 'OPEN');
  assert.ok(calls.some(call => call.some(arg => arg.endsWith(`/commits/${head}/check-runs`)))); assert.ok(calls.some(call => call.includes(`sha=${head}`) && call.includes('PUT')));
});
