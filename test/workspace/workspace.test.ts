...
test('review worktree grants bounded reads with no writable roots', async () => {
  ...
  try {
    ...
    assert.match(await manager.read(review, 'src/reviewed.ts', 1024), /reviewed/); await assert.rejects(manager.write(review, owner, 'src/reviewed.ts', 'bad'), WorkspaceRefusal); await assert.rejects(manager.read(review, 'large.txt', 8), WorkspaceRefusal); await assert.rejects(manager.read(review, '../outside', 1024), WorkspaceRefusal);
